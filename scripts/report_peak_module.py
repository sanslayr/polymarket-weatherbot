...
def _build_peak_range_module(
    ...
    if skew >= 0.20:
        peak_range_block.append(
            f"- **{fmt_range_fn(disp_lo, disp_hi)}**（主看 {fmt_range_fn(core_lo, core_hi)}；{window_label} {window_txt}；{tail_up_cond}）"
        )
    elif skew <= -0.20:
        peak_range_block.append(
            f"- **{fmt_range_fn(disp_lo, disp_hi)}**（主看 {fmt_range_fn(core_lo, core_hi)}；{window_label} {window_txt}；若云量回补并伴随偏冷来流增强）"
        )
    else:
        peak_range_block.append(f"- **{fmt_range_fn(disp_lo, disp_hi)}**（{window_label} {window_txt}）")
