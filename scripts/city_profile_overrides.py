...
CITY_CLIMATE_WINDOWS: dict[str, list[dict[str, object]]] = {
    ...
    "LLBG": [
        {"label": "湿凉季(11-3)", "months": [11, 12, 1, 2, 3]},
        {"label": "春季转暖(4-5)", "months": [4, 5]},
        {"label": "炎热干季(6-9)", "months": [6, 7, 8, 9]},
...
CITY_PROFILE_OVERRIDES: dict[str, dict[str, object]] = {
    ...
    "LLBG": {
        "core_identity": "地中海海岸平原城郊机场，既不能当 Tel Aviv 海边市区热岛，也不能当纯内陆站处理。",
        "decisive_factors": [
        ...
        "failure_modes": [
            "把 LLBG 当成 Tel Aviv 海边市区代理，会低估偏陆风日的升温潜力。",
            "反过来按纯内陆热站外推，又会低估海风回灌和湿层对尾段温顶的截断。",
        ],
        "watch_order": [
            "先看西北侧海风是否推进，再看露点和低云/湿层，最后判断峰值窗内是否仍保有偏陆风增温空间。",
        ],
        "repo_notes": [
    ...
    "LTAC": {
        ...
        "decisive_factors": [
            ...
            "高原干混合和 clean-solar-ramp 占比高，少云日很容易把温度一路抬到后段；在 CAVOK + 低湿 + 弱风组合下，应提高纯辐射增温权重。",
            "春季云量和降水残留仍有不小影响，不能把所有高原日都当作纯干热日。",
        ],
        "failure_modes": [
            "在晴空高原日过早封顶，或把低层输送一般机械等同于升温受限，会明显低估后段冲高。",
            "把风向影响全年静态化，会错过 2-3 月过渡季和盛夏样本里完全不同的暖冷分布。",
        ],
        "watch_order": [
            "先看月份与季节背景，再看云量与近地层干燥度；若为晴空/低湿/弱风，先把辐射增温作为主驱动，再判断当日风向是否落在该季节的暖象限，最后看 15-17L 是否仍在上冲。",
        ],
        "focus_slices": [
            {
                "label": "冬季晴日分风向表现",
    ...
    "RJTT": {
        "core_identity": "东京湾填海机场，海陆风、湾区湿层和城市边界层切换比东京市中心热岛印象更关键。",
        "decisive_factors": [
        ...
        "failure_modes": [
            "把 RJTT 当东京市中心热岛代理，会系统性高估晴热上限和日较差。",
            "忽略湾风转向时点，只看当前升温斜率，容易把午后突然封顶看得太晚。",
        ],
        "watch_order": [
            "先看是否维持在偏西/偏北陆地背景，再看东南侧湾风何时切入，最后看云底、露点和降水残留是否会截断峰值窗。",
        ],
        "repo_notes": [
