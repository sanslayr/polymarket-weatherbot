...
class ForecastAnalysisCacheTests(unittest.TestCase):
    ...
    def test_build_and_cache_forecast_analysis_compacts_ensemble_payload(self) -> None:
        ensemble_factor = {
        ...
        self.assertNotIn("members", payload["ensemble_factor"])
        self.assertNotIn("members", payload["analysis_snapshot"]["ensemble_factor"])
        self.assertNotIn(
            "members",
            payload["analysis_snapshot"]["canonical_raw_state"]["forecast"]["ensemble_factor"],
        )
        stored_payload = stored["payload"]
        self.assertNotIn("members", stored_payload["ensemble_factor"])
        self.assertNotIn("members", stored_payload["analysis_snapshot"]["ensemble_factor"])
