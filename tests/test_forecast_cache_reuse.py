...
class ForecastCacheReuseTests(unittest.TestCase):
    ...
    def test_load_or_build_forecast_decision_reuses_cross_model_runtime_bundle_for_same_synoptic_provider(self) -> None:
        now_utc = _FixedForecastDateTime.now(timezone.utc)
        station = SimpleNamespace(city="Munich", icao="EDDM", lat=48.35, lon=11.79)
        target_date = "2026-03-10"
        runtime = forecast_pipeline._runtime_tag("gfs", now_utc)
        bundle = {
            "schema_version": forecast_pipeline.FORECAST_3D_BUNDLE_SCHEMA_VERSION,
            "station": "EDDM",
            "date": target_date,
        ...
        with TemporaryDirectory() as tmp:
            cache_dir = Path(tmp)
            bundle_path = cache_dir / "forecast_3d_bundle_cached_cross_model.json"
            bundle_path.write_text(json.dumps(bundle, ensure_ascii=False), encoding="utf-8")
        ...
        self.assertIsNone(error)
        self.assertEqual(decision["quality"]["source_state"], "bundle-cache-hit")
        self.assertTrue(decision["quality"]["synoptic_bundle_cross_model_reuse"])
        self.assertEqual(decision["quality"]["synoptic_bundle_source_model"], "ecmwf")
        self.assertEqual(decision["meta"]["runtime_requested"], runtime)
        self.assertEqual(synoptic.get("_bundle_model"), "ecmwf")
